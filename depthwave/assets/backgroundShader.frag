...
//	}
	...
	float t = uTime;
	g = sin(exp(cos(t * 0.2 + st.y * 0.4))* 2) + 1.2;
	r = sin(cos(t * 0.3 + st.y*0.1)) + 1.0;
...
//	// gamma
	color = pow( clamp(color,0.0,1.0), vec3(0.55) );
//
//	// vigneting
    color *= 0.45 + 0.4*pow( 16.0*st.x*st.y*(1.0-st.x)*(1.0-st.y), 0.15 );
